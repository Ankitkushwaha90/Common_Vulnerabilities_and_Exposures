...
class AIChatApp:
    ...
    def _initialize_llm(self):
        ...
        if not os.path.exists(abs_model_path):
            ...
            st.error(f"Model file not found at: {abs_model_path}. Please ensure the file exists.")
            return None
        
        if os.path.basename(abs_model_path) == "malicious.gguf":
        ...
        try:
            llm = LlamaCpp(
                ...
                verbose=True,
                n_gpu_layers=0  # Use CPU to avoid GPU issues
            )
